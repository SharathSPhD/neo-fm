"""
cover-art-synth FastAPI service (v1.3 Sprint 3).

Internal API (never internet-facing). Authenticates with HMAC just like
music-inference and vocal-synth (ADR 0003); structured JSON logs (ADR 0007).

Endpoints:
    GET  /healthz             — service health + model state
    GET  /metrics             — Prometheus exposition
    POST /v1/generate-cover   — render a PNG cover-art square

Model layer is `app.model`; tests substitute `FakeCoverArtModel`.
"""
...
from app.model import CoverArtRequest

PHASE = 6  # v1.3 Sprint 3 lives one phase past the v1.2 vocal-synth (5).
...
def _configure_logger() -> logging.Logger:
    ...
    logger = logging.getLogger("cover-art-synth")
...
def _hmac_secrets() -> list[bytes]:
    out: list[bytes] = []
    primary = os.environ.get("COVER_ART_HMAC_SECRET", "")
    nxt = os.environ.get("COVER_ART_HMAC_SECRET_NEXT", "")
    if primary:
            ...
            if not _hmac_secrets():
                ...
                return JSONResponse(
                    ...
                    content={
                        ...
                        "details": {
                            "message": (
                                "COVER_ART_HMAC_SECRET is not set on this "
                                "cover-art-synth container. See ADR 0003."
                            )
                        },
                    },
                )
...
async def _lifespan(_: FastAPI) -> AsyncIterator[None]:
    if (
        os.environ.get("COVER_ART_SKIP_LIFESPAN") != "1"
...
app = FastAPI(
    title="neo-fm cover-art-synth",
    version="0.1.0",
    description="Internal API. See docs/contracts/openapi-cover-art-synth.yaml.",
    lifespan=_lifespan,
)
...
class GenerateCoverRequest(BaseModel):
    ...
    style_family: str | None = Field(default=None, max_length=64)
...
@app.post(
    ...
    responses={
        200: {
            "description": "PNG image (square, configurable size).",
...
async def generate_cover(req: GenerateCoverRequest, request: Request) -> FastAPIResponse:
    m = model_module.get_active_model()
    if m is None or not m.model_loaded:
        raise HTTPException(
            ...
            detail={
                ...
                "details": {
                    "message": "Cover-art model is not loaded yet.",
                    "phase": PHASE,
                },
            },
        )
    coreq = CoverArtRequest(
        job_id=req.job_id,
    ...
    start = time.perf_counter()
    png_bytes = await asyncio.to_thread(m.synthesise, coreq)
    ...
    request.state.backend = m.backend
    metrics_module.wall_seconds.labels(backend=m.backend).observe(wall)
    metrics_module.set_model_info(
        backend=m.backend,
